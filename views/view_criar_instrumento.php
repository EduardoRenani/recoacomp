...
<div class="disciplinas">
    <div class="top-disciplinas"><?php echo "Atividade: ".$disciplina->getNomeDisciplinaById($_POST['disciplina'])[0]['nomeDisciplina']; ?></div>
        <div class="disciplinas-content">
            <ul class="disciplinas-list" style='text-align: center;'>
                <form action='criar_instrumento.php' method='POST'>
                    <input type='hidden' name='registrar_novo_instrumento'>
            <?php
                echo "<input type='hidden' name='iddisciplina' value='".$_POST['disciplina']."'>";
                echo "<input type='hidden' name='disciplina' value='".$_POST['disciplina']."'>";
                foreach ($_POST['competencias'] as $competencia_disciplina) {
                    echo "<input type='hidden' name='idCompetencias[]' value='".$competencia_disciplina."'>";
                    echo "<input type='hidden' name='competencias[]' value='".$competencia_disciplina."'>";
                    echo "<li class='disciplinas-item' style='text-align: left;'>";
                    echo "<div class='disciplina-item-content'>";
                    echo "<h2>Competência: ".$competencia->getNomeCompetenciaById(intval($competencia_disciplina))[0]['nome']."</h2>";
                    echo "<h4>Primeira situação problema:</h4>";
                    echo "<textarea name='problemasUm[]' placeholder='Elabore aqui...'></textarea>";
                    echo "<h4>Segunda situação problema:</h4>";
                    echo "<textarea name='problemasDois[]' placeholder='Elabore aqui...'></textarea>";
                    echo "<h4>Terceira situação problema:</h4>";
                    echo "<textarea name='problemasTres[]' placeholder='Elabore aqui...'></textarea>";
                    echo "<div>";
                    echo "</li>";
                }
            ?>
...
*/
                ?>
                <input type="submit" value='Criar instrumento'>
                </form>
            </ul>
         </div>  
     </div>
